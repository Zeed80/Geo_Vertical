...
class EnhancedReportGenerator:
    ...
    def _build_verticality_conclusion_lines(vertical_check: dict) -> tuple[str, str]:
        """Возвращает строки заключения по вертикальности для всех форматов отчета."""
        failed = vertical_check.get('failed')
        if failed is None:
            failed = len(vertical_check.get('non_compliant', []))

        has_exceedance = failed > 0
        exceeds_text = "превышают" if has_exceedance else "не превышают"
        line1 = (
            "1. Отклонения ствола от вертикали "
            f"{exceeds_text} допусков СП 70.13330.2012 "
            "«Несущие и ограждающие конструкции. Актуализированная редакция СНиП 3.03.01-87» "
            "(табл.4.15) (0,001H)."
        )
        if has_exceedance:
            line2 = (
                "2. Зафиксированные превышения отклонений ствола от вертикали требуют "
                "дополнительной оценки технического состояния и принятия решения по дальнейшей эксплуатации опоры."
            )
        else:
            line2 = (
    ...
    def _generate_verticality_conclusion(vertical_check: dict) -> str:
        ...
        """
        line1, line2 = EnhancedReportGenerator._build_verticality_conclusion_lines(vertical_check)
    ...
    def generate_professional_pdf(self,
        ...
        try:
            ...
            raise ValueError(f"Ошибка генерации профессионального PDF отчета: {e!s}")
    ...
    def generate_professional_docx(self,
        ...
        try:
            ...
            if verticality_widget and hasattr(verticality_widget, 'figure'):
                ...
                # Заключение после графика вертикальности
                conclusion_text1, conclusion_text2 = self._build_verticality_conclusion_lines(vertical_check)
