...
<xsl:template match="/" >
  ...
  <title>Tunnel Profile Detail Report</title>
  <h2>Tunnel Profile Detail Report</h2>
  ...
  <body>
    ...
    <p>Underbreak/Overbreak values:<br/>
    <font color="red"><b>Red</b></font> values exceed underbreak tolerance<br/>
    <font color="blue"><b>Blue</b></font> values exceed overbreak tolerance</p>

    <p>Delta station values:<br/>
    <font color="red"><b>Red</b></font> values exceed station tolerance</p>
...
<xsl:template match="FieldBook">
  ...
  <xsl:for-each select="TunnelCrossSectionRecord">
    ...
    <xsl:if test="count(following-sibling::TunnelCrossSectionRecord[Station = $stn]) = 0"> <!-- Only output the details for the last profile at this station -->
      ...
      <xsl:call-template name="OutputTableLine">
        <xsl:with-param name="Val1" select="'Point Name'"/>
        <xsl:with-param name="Val2" select="'Underbreak/Overbreak'"/>
        <xsl:with-param name="Val3" select="'Station delta'"/>
      ...
      <xsl:for-each select="msxsl:node-set($profilePoints)/TunnelPointDeltaRecord">
        <xsl:call-template name="OutputTableLine">
          <xsl:with-param name="Val1" select="name"/>
          <xsl:with-param name="Val2" select="format-number(Delta * $DistConvFactor, $DecPl3, 'Standard')"/>
          <xsl:with-param name="Val3" select="format-number(DeltaStation * $DistConvFactor, $DecPl3, 'Standard')"/>
          <xsl:with-param name="Val2Colour">
            <xsl:choose>
              <xsl:when test="undercutExceeded = 'true'">red</xsl:when>
              <xsl:when test="overcutExceeded = 'true'">blue</xsl:when>
              <xsl:otherwise>black</xsl:otherwise>
            </xsl:choose>
          </xsl:with-param>
          <xsl:with-param name="Val3Colour">
            <xsl:choose>
              <xsl:when test="stnTolExceeded = 'true'">red</xsl:when>
              <xsl:otherwise>black</xsl:otherwise>
            </xsl:choose>
          </xsl:with-param>
        </xsl:call-template>
...
<xsl:template name="AssignNodeSetValues">
  ...
  <xsl:if test="string(number(Delta)) != 'NaN'">
    <xsl:copy>
      <xsl:copy-of select="*"/>
...
<xsl:template name="OutputTableLine">
  ...
  <xsl:param name="Val2Colour" select="'black'"/>
  <xsl:param name="Val3Colour" select="'black'"/>

  <tr>
  <xsl:choose>
    <xsl:when test="$hdrLine = 'true'">
      <th width="40%" align="center" bgcolor="silver"><xsl:value-of select="$Val1"/></th>
      <th width="30%" align="center" bgcolor="silver"><xsl:value-of select="$Val2"/></th>
      <th width="30%" align="center" bgcolor="silver"><xsl:value-of select="$Val3"/></th>
    </xsl:when>
    <xsl:otherwise>
      <td width="40%" align="left"><xsl:value-of select="$Val1"/></td>
      <td width="30%" align="right">
        <xsl:value-of disable-output-escaping="yes" select="'&lt;font color=&quot;'"/>
        <xsl:value-of select="$Val2Colour"/>
        <xsl:value-of disable-output-escaping="yes" select="'&quot;&gt;'"/>
        <xsl:if test="$Val2Colour != 'black'">
          <xsl:value-of disable-output-escaping="yes" select="'&lt;b&gt;'"/>   <!-- Bold it -->
        </xsl:if>
        <xsl:value-of select="$Val2"/>
        <xsl:if test="$Val2Colour != 'black'">
          <xsl:value-of disable-output-escaping="yes" select="'&lt;/b&gt;'"/>   <!-- UnBold it -->
        </xsl:if>
        <xsl:value-of disable-output-escaping="yes" select="'&lt;/font&gt;'"/>
      </td>
      <td width="30%" align="right">
        <xsl:value-of disable-output-escaping="yes" select="'&lt;font color=&quot;'"/>
        <xsl:value-of select="$Val3Colour"/>
        <xsl:value-of disable-output-escaping="yes" select="'&quot;&gt;'"/>
        <xsl:if test="$Val3Colour != 'black'">
          <xsl:value-of disable-output-escaping="yes" select="'&lt;b&gt;'"/>   <!-- Bold it -->
        </xsl:if>
