...
<xsl:variable name="userField1" select="'reportedCoordsFrom|Report point coordinates from|stringMenu|2|MTA observations|Reductions section'"/>
<xsl:variable name="reportedCoordsFrom" select="'MTA observations'"/>
...
<xsl:template match="/" >
  ...
  <title>ISO Rounds Report</title>
  <h3>ISO Rounds Report</h3>
  ...
  <body>
    <xsl:call-template name="OutputHeadings"/>
    <xsl:call-template name="OutputStationData"/>
  </body>
  </html>
</xsl:template>
...
<!-- **************************************************************** -->
<xsl:template name="OutputStationData">
  <xsl:for-each select="/JOBFile/FieldBook/StationRecord">
    <xsl:call-template name="Heading1">
      <xsl:with-param name="text">Instrument Station</xsl:with-param>
    </xsl:call-template>
    ...
    <xsl:call-template name="StartTable">
      ...
      <xsl:call-template name="OutputOneElementTableLine">
        <xsl:with-param name="hdr">Stationing Type</xsl:with-param>
        <xsl:with-param name="val">
          ...
          <xsl:if test="StationType[.='StandardResection']">Standard Resection</xsl:if>
          <xsl:if test="StationType[.='HelmertResection']">Helmert Resection</xsl:if>
          <xsl:if test="StationType[.='ReflineStationSetup']">Refline Station Setup</xsl:if>
          <xsl:if test="StationType[.='Unknown']">Unknown</xsl:if>
        </xsl:with-param>
      </xsl:call-template>
    <xsl:call-template name="EndTable"/>
    ...
    <xsl:call-template name="StartTable">
      ...
      <xsl:call-template name="OutputTwoElementTableLine">
        <xsl:with-param name="hdr2">Atmospheric Settings</xsl:with-param>
      </xsl:call-template>
        ...
        <xsl:with-param name="val2">
          <xsl:for-each select="key('atmosID-search', AtmosphereID)">
            <xsl:variable name="tempVal">
              <xsl:call-template name="TemperatureValue">
                <xsl:with-param name="temperature" select="Temperature"/>
              </xsl:call-template>
            </xsl:variable>
            <xsl:value-of select="concat(format-number($tempVal, $DecPl1, 'Standard'), $TempUnitStr)"/>
          </xsl:for-each>
        </xsl:with-param>
      </xsl:call-template>

      <xsl:call-template name="OutputTwoElementTableLine">
        <xsl:with-param name="hdr1">Code</xsl:with-param>
        <xsl:with-param name="val1">  <!-- Get any station code from the Reductions section -->
          <xsl:for-each select="/JOBFile/Reductions/Point[Name = $stnName]">
            <xsl:value-of select="Code"/>
          </xsl:for-each>
        </xsl:with-param>
        <xsl:with-param name="hdr2">Pressure</xsl:with-param>
        <xsl:with-param name="val2">
          <xsl:for-each select="key('atmosID-search', AtmosphereID)">
            <xsl:value-of select="concat(format-number(Pressure * $PressConvFactor, $DecPl1, 'Standard'), $PressUnitStr)"/>
          </xsl:for-each>
        </xsl:with-param>
      </xsl:call-template>

      <xsl:call-template name="OutputTwoElementTableLine">
        <xsl:with-param name="hdr1">Instrument Height</xsl:with-param>
        <xsl:with-param name="val1" select="format-number(TheodoliteHeight * $DistConvFactor, $DecPl3, 'Standard')"/>
        <xsl:with-param name="hdr2">Refraction Const.</xsl:with-param>
        <xsl:with-param name="val2">
          <xsl:for-each select="key('atmosID-search', AtmosphereID)">
            <xsl:choose>
              <xsl:when test="RefractionCoefficient">
                <xsl:value-of select="format-number(RefractionCoefficient, $DecPl3, 'Standard')"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="format-number(Refraction, $DecPl3, 'Standard')"/>
              </xsl:otherwise>
            </xsl:choose>
          </xsl:for-each>
        </xsl:with-param>
    ...
    <xsl:for-each select="preceding-sibling::InstrumentRecord[1]">
      <xsl:call-template name="StartTable">
        <xsl:with-param name="includeBorders">false</xsl:with-param>
        <xsl:with-param name="tableWidth" select="50"/>
      </xsl:call-template>
        <xsl:call-template name="OutputOneElementTableLine">
          ...
          <xsl:with-param name="val">
            <xsl:if test="SetHorizontalCircleMode[.='DoNothing']">Do Nothing</xsl:if>
            <xsl:if test="SetHorizontalCircleMode[.='Zero']">Zero</xsl:if>
            <xsl:if test="SetHorizontalCircleMode[.='SetToAzimuth']">Set To Azimuth</xsl:if>
            <xsl:if test="SetHorizontalCircleMode[.='SetHorizontalNull']">Set Horizontal Null</xsl:if>
          </xsl:with-param>
        </xsl:call-template>
      <xsl:call-template name="EndTable"/>
      ...
      <xsl:call-template name="StartTable">
        ...
        <xsl:call-template name="OutputOneElementTableLine">
          <xsl:with-param name="hdr">Standard Deviations</xsl:with-param>
        </xsl:call-template>

        <xsl:call-template name="OutputOneElementTableLine">
          <xsl:with-param name="hdr">Hz</xsl:with-param>
          <xsl:with-param name="val">
            <xsl:call-template name="FormatAngle">
              <xsl:with-param name="theAngle" select="HorizontalAnglePrecision"/>
            </xsl:call-template>
            <xsl:value-of select="$AngleUnitStr"/>
          </xsl:with-param>
        </xsl:call-template>

        <xsl:call-template name="OutputOneElementTableLine">
          <xsl:with-param name="hdr">V</xsl:with-param>
          <xsl:with-param name="val">
            <xsl:call-template name="FormatAngle">
        ...
        <xsl:call-template name="OutputOneElementTableLine">
          ...
          <xsl:with-param name="val" select="concat(format-number(EDMPrecision * 1000, $DecPl0, 'Standard'), 'mm')"/>
        </xsl:call-template>

        <xsl:call-template name="OutputOneElementTableLine">
          <xsl:with-param name="hdr">EDM ppm</xsl:with-param>
          <xsl:with-param name="val" select="concat(format-number(EDMppm, $DecPl0, 'Standard'), 'ppm')"/>
        </xsl:call-template>

        <xsl:if test="InstrumentAppliedEDMConstant and (string(number(InstrumentAppliedEDMConstant)) != 'NaN')">
          <xsl:call-template name="OutputOneElementTableLine">
            <xsl:with-param name="hdr">Instrument Applied EDM Constant</xsl:with-param>
            <xsl:with-param name="val" select="concat(format-number(InstrumentAppliedEDMConstant * 1000, $DecPl0, 'Standard'), 'mm')"/>
          </xsl:call-template>
        </xsl:if>

        <xsl:call-template name="OutputOneElementTableLine">
          <xsl:with-param name="hdr">Centering Error</xsl:with-param>
          <xsl:with-param name="val" select="concat(format-number(CenteringError * 1000, $DecPl0, 'Standard'), 'mm')"/>
        </xsl:call-template>

        <xsl:if test="BacksightCenteringError">
          <xsl:call-template name="OutputOneElementTableLine">
            <xsl:with-param name="hdr">Backsight Centering Error</xsl:with-param>
            <xsl:with-param name="val" select="concat(format-number(BacksightCenteringError * 1000, $DecPl0, 'Standard'), 'mm')"/>
          </xsl:call-template>
        </xsl:if>

      <xsl:call-template name="EndTable"/>
    </xsl:for-each>  <!-- preceding-sibling::InstrumentRecord[1] -->

    <xsl:call-template name="SeparatingLine"/>

    <xsl:call-template name="Heading2">
      <xsl:with-param name="text">Backsight Points</xsl:with-param>
    </xsl:call-template>

    <xsl:call-template name="OutputBacksightPoints">
      <xsl:with-param name="stationID" select="@ID"/>
    </xsl:call-template>

    <xsl:call-template name="SeparatingLine"/>

    <xsl:call-template name="Heading2">
      <xsl:with-param name="text">Measurements</xsl:with-param>
    </xsl:call-template>

    <xsl:call-template name="OutputBacksightMeasurements">
      <xsl:with-param name="stationID" select="@ID"/>
    </xsl:call-template>

    <xsl:if test="(StationType = 'StationSetupPlus') or
                  (StationType = 'StandardResection') or
                  (StationType = 'HelmertResection')">
      <!-- There will be station residuals available for output -->
      <xsl:call-template name="Heading2">
        <xsl:with-param name="text">Residuals</xsl:with-param>
      </xsl:call-template>

      <xsl:call-template name="OutputStationResiduals"/>

      <xsl:call-template name="SeparatingLine"/>

    </xsl:if>

    <!-- Output the station position result, orientation and scale factor -->
    <xsl:call-template name="Heading2">
      <xsl:with-param name="text">Result</xsl:with-param>
    </xsl:call-template>

    <xsl:variable name="resectedStn">
      <xsl:choose>
        <xsl:when test="(StationType = 'StandardResection') or
                        (StationType = 'HelmertResection')">true</xsl:when>
        <xsl:otherwise>false</xsl:otherwise>
      </xsl:choose>
    </xsl:variable>

    <xsl:call-template name="OutputStationPosition">
      <xsl:with-param name="stnName" select="$stnName"/>
      <xsl:with-param name="scale" select="ScaleFactor"/>
      <xsl:with-param name="scaleSE" select="ScaleFactorStandardError"/>
      <xsl:with-param name="resectedStn" select="$resectedStn"/>
    </xsl:call-template>
    ...
    <xsl:for-each select="following-sibling::*">
      <xsl:if test="name(.) = 'StartRoundsRecord'">
        <xsl:if test="(position() &gt; 0) and
          ...
          <!-- We have a set of rounds observed at the current station - output the details -->
          <xsl:call-template name="Heading1">
            <xsl:with-param name="text">Multiple Rounds</xsl:with-param>
          </xsl:call-template>
...
<xsl:template name="OutputRoundsData">
  ...
  <!-- Now write the assembled horizonta; angle data out to the output tables -->
  <xsl:call-template name="OutputHorizAngleTableData">
    <xsl:with-param name="stnName" select="$stnName"/>
    <xsl:with-param name="obsData" select="$obsData"/>
    <xsl:with-param name="HorizMeanOfRounds" select="$HorizMeanOfRounds"/>
    <xsl:with-param name="HorizDiffsFromMeanPerRound" select="$HorizDiffsFromMeanPerRound"/>
    <xsl:with-param name="HorizResiduals" select="$HorizResiduals"/>
  </xsl:call-template>
  ...
  <xsl:call-template name="OutputAveragedDataSets">
    <xsl:with-param name="stnName" select="$stnName"/>
    <xsl:with-param name="instHt" select="$instHt"/>
    <xsl:with-param name="obsData" select="$obsData"/>
    <xsl:with-param name="HorizMeanOfRounds" select="$HorizMeanOfRounds"/>
...
<xsl:template name="OutputAveragedDataSets">
  ...
  <xsl:call-template name="Heading2">
    <xsl:with-param name="text">4. Averaged Data Sets</xsl:with-param>
  </xsl:call-template>
  ...
  <xsl:call-template name="StartTable">
    ...
    <!-- Write out the first table section headings -->
    <xsl:call-template name="OutputNineElementTableLine">
      <xsl:with-param name="val1">Point Name</xsl:with-param>
      <xsl:with-param name="val2">Code</xsl:with-param>
      <xsl:with-param name="val3">HA</xsl:with-param>
      <xsl:with-param name="val4">VA</xsl:with-param>
      <xsl:with-param name="val5">SD</xsl:with-param>
      <xsl:with-param name="val6">ih/th</xsl:with-param>
      <xsl:with-param name="val7">PC</xsl:with-param>
      <xsl:with-param name="val8">Temp [<xsl:value-of select="$TempUnitStr"/>]</xsl:with-param>
      <xsl:with-param name="val9">Press [<xsl:value-of select="$PressUnitStr"/>]</xsl:with-param>
      <xsl:with-param name="bold">true</xsl:with-param>
    </xsl:call-template>

    <!-- Output first line with station details -->
    <xsl:call-template name="OutputNineElementTableLine">
      <xsl:with-param name="val1"><xsl:value-of select="$stnName"/></xsl:with-param>
      <xsl:with-param name="val2">
        <xsl:for-each select="key('reducedPt-search', $stnName)">
          <xsl:value-of select="Code"/>
        </xsl:for-each>
      </xsl:with-param>
      <xsl:with-param name="val6"><xsl:value-of select="format-number($instHt * $DistConvFactor, $DecPl3, 'Standard')"/></xsl:with-param>
    </xsl:call-template>
    ...
    <xsl:for-each select="msxsl:node-set($HorizMeanOfRounds)/Point">
      ...
      <xsl:call-template name="OutputNineElementTableLine">
        <!-- Point name -->
        <xsl:with-param name="val1"><xsl:value-of select="Name"/></xsl:with-param>
        <!-- Point code -->
        <xsl:with-param name="val2"><xsl:value-of select="msxsl:node-set($obsData)/Round[1]/Point[Name = $ptName]/Code"/></xsl:with-param>
        <!-- Mean horizontal angle -->
        <xsl:with-param name="val3">
          <xsl:call-template name="FormatAngle">
            <xsl:with-param name="theAngle" select="roundsMeanHz"/>
          </xsl:call-template>
        </xsl:with-param>
        <!-- Mean vertical angle -->
        <xsl:with-param name="val4">
        ...
        <!-- Mean slope distance -->
        <xsl:with-param name="val5" select="format-number(msxsl:node-set($DistMeanOfRounds)/Point[Name = $ptName]/roundsMeanDist * $DistConvFactor, $DecPl4, 'Standard')"/>
        <!-- Target height -->
        <xsl:with-param name="val6" select="format-number(msxsl:node-set($obsData)/Round[1]/Point[Name = $ptName]/tgtHt * $DistConvFactor, $DecPl3, 'Standard')"/>
        <!-- Prism constant - in mm -->
        <xsl:with-param name="val7" select="format-number(msxsl:node-set($obsData)/Round[1]/Point[Name = $ptName]/prismConst * 1000.0, $DecPl1, 'Standard')"/>
        <!-- Temperature -->
        <xsl:with-param name="val8">
          <xsl:variable name="temp">
            <xsl:call-template name="TemperatureValue">
              <xsl:with-param name="temperature" select="msxsl:node-set($obsData)/Round[1]/Point[Name = $ptName]/Temperature"/>
            </xsl:call-template>
          </xsl:variable>
          <xsl:value-of select="format-number($temp, $DecPl1, 'Standard')"/>
        </xsl:with-param>
        <!-- Pressure -->
        <xsl:with-param name="val9" select="format-number(msxsl:node-set($obsData)/Round[1]/Point[Name = $ptName]/Pressure * $PressConvFactor, $DecPl1, 'Standard')"/> <!-- Output prism constant in mm -->
      </xsl:call-template>
    </xsl:for-each>

    <!-- Output a blank line in the table -->
    <xsl:call-template name="OutputNineElementTableLine">
    ...
    <!-- Write out the second table section headings -->
    <xsl:call-template name="OutputNineElementTableLine">
      <xsl:with-param name="val1">Point Name</xsl:with-param>
      <xsl:with-param name="val2">Code</xsl:with-param>
      <xsl:with-param name="val3">Az</xsl:with-param>
      <xsl:with-param name="val4">VA</xsl:with-param>
      <xsl:with-param name="val5">SD</xsl:with-param>
      <xsl:with-param name="val6">ih/th</xsl:with-param>
      <xsl:with-param name="bold">true</xsl:with-param>
    ...
    <xsl:for-each select="msxsl:node-set($HorizMeanOfRounds)/Point">
      ...
      <xsl:call-template name="OutputNineElementTableLine">
        ...
        <!-- Point code -->
        <xsl:with-param name="val2"><xsl:value-of select="msxsl:node-set($obsData)/Round[1]/Point[Name = $ptName]/Code"/></xsl:with-param>
        <!-- Azimuth (based on the mean horizontal angle applied to the computed azimuth to the first point) -->
        <xsl:with-param name="val3">
          <xsl:variable name="azimuth">
            <xsl:call-template name="NormalisedAngle">
              <xsl:with-param name="angle" select="$azToFirstPt + roundsMeanHz"/>
            </xsl:call-template>
          </xsl:variable>
    ...
    <!-- Write out the third table section headings -->
    <xsl:call-template name="OutputNineElementTableLine">
      <xsl:with-param name="val1">Point Name</xsl:with-param>
      <xsl:with-param name="val2">Code</xsl:with-param>
      <xsl:with-param name="val3">Az</xsl:with-param>
      <xsl:with-param name="val4">HD</xsl:with-param>
      <xsl:with-param name="val5">VD</xsl:with-param>
      <xsl:with-param name="bold">true</xsl:with-param>
    </xsl:call-template>
    ...
    <xsl:for-each select="msxsl:node-set($HorizMeanOfRounds)/Point">
      ...
      <xsl:call-template name="OutputNineElementTableLine">
        ...
        <xsl:with-param name="val4">
          ...
          <xsl:value-of select="format-number($sineVA * msxsl:node-set($DistMeanOfRounds)/Point[Name = $ptName]/roundsMeanDist * $DistConvFactor, $DecPl3, 'Standard')"/>
        </xsl:with-param>
        <!-- Vertical distance -->
        <xsl:with-param name="val5">
          ...
          <xsl:value-of select="format-number(($rawVD + $instHt - $tgtHt) * $DistConvFactor, $DecPl3, 'Standard')"/>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:for-each>
    ...
    <!-- Write out the fourth table section headings -->
    <xsl:call-template name="OutputNineElementTableLine">
      <xsl:with-param name="val1">Point Name</xsl:with-param>
      <xsl:with-param name="val2">Code</xsl:with-param>
      <xsl:with-param name="val3">
        <xsl:if test="$NECoords = 'true'">Northing</xsl:if>
        <xsl:if test="$NECoords != 'true'">Easting</xsl:if>
      </xsl:with-param>
      <xsl:with-param name="val4">
        <xsl:if test="$NECoords = 'true'">Easting</xsl:if>
        <xsl:if test="$NECoords != 'true'">Northing</xsl:if>
      </xsl:with-param>
      <xsl:with-param name="val5">Elevation</xsl:with-param>
      <xsl:with-param name="bold">true</xsl:with-param>
    </xsl:call-template>
    ...
    <xsl:call-template name="OutputNineElementTableLine">
      ...
      <xsl:with-param name="val3">
        <xsl:if test="$NECoords = 'true'">
          <xsl:for-each select="key('reducedPt-search', $stnName)">
            <xsl:value-of select="format-number(Grid/North * $DistConvFactor, $DecPl4, 'Standard')"/>
          </xsl:for-each>
        </xsl:if>
        <xsl:if test="$NECoords != 'true'">
          <xsl:for-each select="key('reducedPt-search', $stnName)">
            <xsl:value-of select="format-number(Grid/East * $DistConvFactor, $DecPl4, 'Standard')"/>
          </xsl:for-each>
        </xsl:if>
      </xsl:with-param>
      ...
      <xsl:with-param name="val5">
        <xsl:for-each select="key('reducedPt-search', $stnName)">
          <xsl:value-of select="format-number(Grid/Elevation * $DistConvFactor, $DecPl4, 'Standard')"/>
        </xsl:for-each>
      </xsl:with-param>
    </xsl:call-template>

    <!-- Put all the reduced point coordinates into a variable so that they can be accessed in the loop below -->
    <xsl:variable name="reducedPts">
      <xsl:for-each select="/JOBFile/Reductions/Point">
        ...
        <xsl:element name="Point">
          ...
          <xsl:element name="North">
            <xsl:choose>
              <xsl:when test="($reportedCoordsFrom = 'MTA observations') and /JOBFile/FieldBook/PointRecord[(Name = $ptName) and MTA][last()]">
                <xsl:value-of select="/JOBFile/FieldBook/PointRecord[(Name = $ptName) and MTA][last()]/ComputedGrid/North"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="Grid/North"/>
              </xsl:otherwise>
          ...
          <xsl:element name="East">
            <xsl:choose>
              <xsl:when test="($reportedCoordsFrom = 'MTA observations') and /JOBFile/FieldBook/PointRecord[(Name = $ptName) and MTA][last()]">
                <xsl:value-of select="/JOBFile/FieldBook/PointRecord[(Name = $ptName) and MTA][last()]/ComputedGrid/East"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="Grid/East"/>
              </xsl:otherwise>
          ...
          <xsl:element name="Elev">
            <xsl:choose>
              <xsl:when test="($reportedCoordsFrom = 'MTA observations') and /JOBFile/FieldBook/PointRecord[(Name = $ptName) and MTA][last()]">
                <xsl:value-of select="/JOBFile/FieldBook/PointRecord[(Name = $ptName) and MTA][last()]/ComputedGrid/Elevation"/>
              </xsl:when>
              <xsl:otherwise>
                <xsl:value-of select="Grid/Elevation"/>
              </xsl:otherwise>
    ...
    <xsl:for-each select="msxsl:node-set($HorizMeanOfRounds)/Point">
      ...
      <xsl:call-template name="OutputNineElementTableLine">
        ...
        <xsl:with-param name="val3">
          <xsl:if test="$NECoords = 'true'">
            <xsl:for-each select="msxsl:node-set($reducedPts)/Point[Name = $ptName]">
              <xsl:value-of select="format-number(North * $DistConvFactor, $DecPl4, 'Standard')"/>
            </xsl:for-each>
          </xsl:if>
          <xsl:if test="$NECoords != 'true'">
            <xsl:for-each select="msxsl:node-set($reducedPts)/Point[Name = $ptName]">
              <xsl:value-of select="format-number(East * $DistConvFactor, $DecPl4, 'Standard')"/>
            </xsl:for-each>
          </xsl:if>
        </xsl:with-param>
        ...
        <xsl:with-param name="val5">
          <xsl:for-each select="msxsl:node-set($reducedPts)/Point[Name = $ptName]">
            <xsl:value-of select="format-number(Elev * $DistConvFactor, $DecPl4, 'Standard')"/>
          </xsl:for-each>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:for-each>
...
<xsl:template name="StartTable">
  ...
  <xsl:choose>
    <xsl:when test="$includeBorders = 'true'">
      <xsl:value-of disable-output-escaping="yes" select="concat('&lt;table border=&quot;1&quot; width=&quot;', $tableWidth, '%&quot; cellpadding=&quot;2&quot; rules=&quot;all&quot;&gt;')"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of disable-output-escaping="yes" select="concat('&lt;table border=&quot;0&quot; width=&quot;', $tableWidth, '%&quot; cellpadding=&quot;2&quot; rules=&quot;none&quot;&gt;')"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
