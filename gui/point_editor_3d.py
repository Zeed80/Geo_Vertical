...
class ButtonGroupWidget(QWidget):
    """Виджет-группа для кнопок с рамкой и заголовком снизу (как в Microsoft Word)"""
    ...
    def __init__(self, title: str, parent: Optional[QWidget] = None):
        ...
        # Контейнер для кнопок в ряд (строго друг за другом)
        ...
        # Стиль рамки с отличительным фоном
        self.setStyleSheet("""
            QWidget#buttonGroup {
    ...
    def buttons(self):
        """Получить список всех кнопок в группе"""
        buttons = []
    ...
    def sizeHint(self):
        """Вычисляет рекомендуемый размер группы"""
        # Вычисляем ширину всех кнопок
        total_width = 0
        max_height = 0
        for i in range(self.buttons_layout.count()):
            item = self.buttons_layout.itemAt(i)
        ...
        height = max_height + margins.top() + margins.bottom() + 20  # +20 для заголовка
...
class ToolPanelWidget(QWidget):
    ...
    def add_group(self, group: ButtonGroupWidget):
        """Добавить группу кнопок на панель."""
        if group is None:
            return
    ...
    def set_position(self, position: str):
        """Задать расположение панели: left, right, top."""
        ...
        # Сбрасываем возможные фиксированные размеры перед пересчётом
    ...
    def reflow(self):
        """Пересобирает сетку кнопок в зависимости от размеров и позиции."""
        if not self._items:
            return
        ...
        for widget in self._items:
            # Для групп кнопок используем sizeHint
...
class PointEditDialog(QDialog):
    ...
    def _validate_coordinate(self, edit: QLineEdit, coord_name: str) -> bool:
        """
        Валидация координаты в реальном времени.
        
        Args:
            edit: QLineEdit для координаты
            coord_name: Имя координаты ('X', 'Y', 'Z')
        ...
        # Пустое значение - допустимо (будет проверено при сохранении)
        if not text:
            edit.setStyleSheet('')
            return False
        ...
        try:
            ...
            # Специальная валидация для Z (высота не может быть отрицательной)
    ...
    def get_point_data(self) -> Optional[dict]:
        """
        Получить обновленные данные точки с валидацией.
        
        Returns:
            Словарь с данными точки или None, если валидация не прошла
        """
        ...
        try:
            ...
            # Финальная проверка на разумные значения
    ...
    def __init__(
        ...
        *,
        ...
        for info in self.section_infos:
            ...
            # Формируем текст с информацией: № секции, общая высота от 0, часть башни, секция
...
class PointEditor3DWidget(QWidget):
        ...
        try:
            ...
            for pos_idx in range(len(self.data)):
                ...
                if 'point_index' in self.data.columns:
                    try:
                        point_index_value = int(row['point_index'])
                    except (TypeError, ValueError):
                        point_index_value = None
                if point_index_value is not None and point_index_value >= 0:
                    if raw_name:
                        point_name = f'#{point_index_value} {raw_name}'
                    else:
                        point_name = f'#{point_index_value}'
                elif raw_name:
                    point_name = raw_name
                else:
                    point_name = f'Точка {dataframe_idx}'

                text_pos = (float(row['x']), float(row['y']), float(row['z']) + offset_z)
                font = pg.QtGui.QFont('Arial', 12, pg.QtGui.QFont.Weight.Bold)
                text_item = self._create_text_item(
                    position=text_pos,
                    text=point_name,
    ...
    def update_belt_lines(self):
        ...
        for belt_num, belt_data in belts:
            ...
            top_point = line_points[-1]
            belt_label = self._create_text_item(
                position=(float(top_point[0]), float(top_point[1]), float(top_point[2]) + 0.5),
    ...
    def find_belt_line_at_position(self, mouse_pos) -> Optional[Tuple[int, int, int]]:
        ...
        try:
            ...
            for belt_num in available_belts:
                ...
                center_x = belt_points_data['x'].mean()
                center_y = belt_points_data['y'].mean()
                
                angles = np.arctan2(
                    belt_points_data['y'] - center_y,
                    belt_points_data['x'] - center_x
                )
                sorted_idx = np.argsort(angles)
                sorted_indices = [belt_indices[i] for i in sorted_idx]
    ...
    def _create_text_item(self, position, text, color=(1.0, 1.0, 1.0, 1.0), font=None):
