...
def export_data(
    ...
    **kwargs
    ...
    try:
        ...
        raise ExportError(f"Ошибка экспорта в {format}: {e!s}") from e
...
def export_data_to_csv_enhanced(
    ...
    include_results: dict[str, Any] | None = None
) -> None:
    """
    ...
    try:
        with open(file_path, 'w', encoding='utf-8-sig', newline='') as f:
            ...
            if include_metadata:
                f.write("# GeoVertical Analyzer Export\n")
        ...
        raise ExportError(f"Ошибка экспорта в CSV: {e!s}") from e
...
def export_data_to_shapefile(
    ...
    try:
        ...
        raise ExportError(f"Ошибка экспорта в Shapefile: {e!s}") from e
