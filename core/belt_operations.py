...
def validate_belt_geometry(points: pd.DataFrame, 
    ...
    try:
        ...
        return False, f"Ошибка аппроксимации окружности: {str(e)}"
...
def auto_assign_belts(points: pd.DataFrame, 
                     expected_belt_count: Optional[int] = None,
    ...
    if expected_belt_count is None or expected_belt_count <= 0:
        ...
        # Назначаем пояса
        result['belt'] = [final_map.get(l, None) for l in labels]
...
def detect_instrument_station(points: pd.DataFrame) -> Optional[int]:
    """
