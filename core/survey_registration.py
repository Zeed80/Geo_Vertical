...
def helmert_transform_points(points: np.ndarray, params: np.ndarray) -> np.ndarray:
    ...
    logger.info(f"=== НАЧАЛО ПРЕОБРАЗОВАНИЯ ГЕЛЬМЕРТА ===")
    ...
    logger.info(f"Распарсенные параметры:")
    ...
    logger.info(f"Преобразование применено успешно")
...
def apply_helmert_transform(points: pd.DataFrame, transform_params: Dict, rotation_center: Optional[np.ndarray] = None) -> pd.DataFrame:
    """
    ...
    """
    # Детальное логирование для отладки
    logger.info(f"=== НАЧАЛО ПРИМЕНЕНИЯ ПРЕОБРАЗОВАНИЯ ГЕЛЬМЕРТА ===")
    logger.info(f"Входные параметры:")
...
def register_belt_survey(
    ...
    all_points_second_survey: Optional[pd.DataFrame] = None
) -> Dict:
    """
    ...
    """
    # Детальное логирование для отладки
    logger.info(f"=== НАЧАЛО РЕГИСТРАЦИИ ПОЯСА ===")
    logger.info(f"Входные параметры:")
    ...
    if all_points_second_survey is not None and len(all_points_second_survey) > 0:
        ...
        logger.info(f"Используется центр ЦЕЛЕВОГО ПОЯСА для трансформации (не всей съемки)")
...
def compute_rotation_from_plane(
    ...
    logger.info(f"Вычисление поворота по плоскости:")
...
def compute_rotation_from_belt_connections(
    ...
    logger.info(f"Вычисление поворота на основе соединения с поясами:")
    logger.info(f"  Точка из первой съемки: ({p1[0]:.3f}, {p1[1]:.3f}, {p1[2]:.3f})")
    logger.info(f"  Точка из второй съемки: ({p2[0]:.3f}, {p2[1]:.3f}, {p2[2]:.3f})")
...
def compute_rotation_from_connected_points(
    ...
    logger.info(f"Вычисление поворота на основе соединения точек:")
    logger.info(f"  Точка из первой съемки: ({p1[0]:.3f}, {p1[1]:.3f}, {p1[2]:.3f})")
    logger.info(f"  Точка из второй съемки: ({p2[0]:.3f}, {p2[1]:.3f}, {p2[2]:.3f})")
...
def compute_xy_signed_angle(v1_xy: np.ndarray, v2_xy: np.ndarray, direction: int) -> float:
    ...
    v1 = np.array([v1_xy[0], v1_xy[1]])
    v2 = np.array([v2_xy[0], v2_xy[1]])
