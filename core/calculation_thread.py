...
class CalculationThread(QThread):
    ...
    def __init__(
        ...
        self.section_grouping_mode = section_grouping_mode
    ...
    def run(self):
        ...
        try:
            ...
            results = process_tower_data(
                ...
                section_grouping_mode=self.section_grouping_mode,
                use_cache=True
            )
            ...
            error_msg = f"Ошибка расчета: {e!s}"
            logger.error(error_msg, exc_info=True)
            self.error.emit(error_msg)
        except Exception as e:
            error_msg = f"Неожиданная ошибка при выполнении расчетов: {e!s}"
            logger.error(error_msg, exc_info=True)
            self.error.emit(error_msg)
        finally:
            self.finished.emit()
