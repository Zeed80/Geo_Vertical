...
class TowerModelBuilder:
    def __init__(self, blueprint: TowerBlueprintV2, profile_manager: ProfileManager | None = None):
        self.blueprint = blueprint
        self.profile_manager = profile_manager or ProfileManager()

    def _resolve_profile(self, designation: str) -> dict | None:
    ...
    def build(self) -> TowerModel:
        ...
        for i in range(faces):
            ...
            raw_base_nodes.append((r * np.cos(angle), r * np.sin(angle)))

        for point_idx in clockwise_order_indices(np.array(raw_base_nodes, dtype=float)):
            x, y = raw_base_nodes[int(point_idx)]
            # Fixed nodes at Z=0
        ...
        for segment in self.blueprint.segments:
            ...
            for section in sections:
                ...
                for i in range(faces):
                    angle = 2 * np.pi * i / faces + rot_rad
                    raw_curr_nodes.append((r_curr * np.cos(angle), r_curr * np.sin(angle)))

                for point_idx in clockwise_order_indices(np.array(raw_curr_nodes, dtype=float)):
                    x, y = raw_curr_nodes[int(point_idx)]
