...
def _line_intersection_2d(p: np.ndarray, d: np.ndarray, q: np.ndarray, e: np.ndarray) -> Optional[np.ndarray]:
    """
...
def complete_missing_belt_parallel(points: pd.DataFrame,
    ...
    def sort_belt(belt_df: pd.DataFrame) -> pd.DataFrame:
        center = belt_df[['x','y']].mean().values
        angles = np.arctan2(belt_df['y'].values - center[1], belt_df['x'].values - center[0])
...
def _build_perpendicular_line(point_xy: np.ndarray, bisector: np.ndarray, center_xy: Optional[np.ndarray] = None) -> np.ndarray:
    """
...
def _intersect_line_with_polyline(p: np.ndarray, d: np.ndarray, polyline: np.ndarray, eps: float = 1e-6) -> Optional[np.ndarray]:
    """
...
def complete_missing_belt_parallel_lines(points: pd.DataFrame,
    ...
    if 1 not in neighbor_belts:
        logger.info(f"[parallel_lines] Пояс 1 не является соседним, но будет использован как эталон для высот")
